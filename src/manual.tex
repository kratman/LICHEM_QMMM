...
Most of the development of FLUKE should be done though git (or other version
control packages) and merged into the main repository. Since FLUKE does not
require a large number of libraries or proprietary software, it should be easy
to share new wrappers and approximations with the community. \\

When functionality is added to FLUKE, the goal should be to make it simple and
...
Ultra-fine integrals and quadratic convergence (YQC) is probably overkill for
most QM calculations, however, these options assure that difficult cases will
converge. Additionally, the YQC and "Big" options are helpful for larger
systems, but they are not disabled for small systems. Most of the wrappers
contain assumptions that the QMMM calculations will be difficult to simulate
and quite large. These sorts of options ensure that FLUKE will work on all
systems without requiring the user to be an expert with the QM and MM
software. \\

FLUKE should be tested with extremely difficult systems. During the early
stages of the development, test systems with highly distorted structures were
employed to make sure that crashing the software was difficult.
...
user:\$ mkdir FLUKE \\
user:\$ cd FLUKE/ \\
user:\$ unzip FLUKE\_QMMM-master.zip \\
user:\$ mv FLUKE\_QMMM-master/* . \\
user:\$ rmdir FLUKE\_QMMM-master \\

The FLUKE binary will eventually be included with the zipped source code,
however, modified or git versions can be compiled with the script provided
with FLUKE. On Ubuntu boxes, the compile script should function without
...
\begin{table}[hbt]
 ...
 \hline
  & Gaussian & PSI4 & NWChem & TINKER & AMBER & LAMMPS \\ \hline
 QM energy & Yes & Yes & Yes & No & No & No \\
 MM energy & No & No & No & Yes & Yes & Yes \\
 QMMM energy & Yes & Yes & Yes & Yes & Yes & Yes \\ \hline
 QM opt. & Yes & Yes & No & No & No & No \\
 MM opt. & No & No & No & Yes & Yes & Yes \\
 QMMM opt. & Yes & No & No & Yes & Yes & Yes \\ \hline
 QM SD/DFP & Yes & Yes & Yes & No & No & No \\
 MM SD/DFP & No & No & No & No & No & No \\
 QMMM SD/DFP & Yes & No & Yes & Yes & Yes & Yes \\ \hline
 QM MC & Yes & Yes & Yes & No & No & No \\
 MM MC & No & No & No & Yes & Yes & Yes \\
 QMMM MC & Yes & Yes & Yes & Yes & Yes & Yes \\ \hline
 QM PIMC & Yes & Yes & Yes & No & No & No \\
 MM PIMC & No & No & No & Yes & Yes & Yes \\
 QMMM PIMC & Yes & Yes & Yes & Yes & Yes & Yes \\ \hline
 QM RP & Yes & No & Yes & No & No & No \\
 MM RP & No & No & no & No & No & No \\
 QMMM RP & Yes & No & Yes & Yes & Yes & Yes \\ \hline
 \end{tabular}
 \caption{Wrapper capabilities for single-point energy, geometry optimization,
 steepest descent or Davidson-Fletcher-Powell (SD/DFP), Monte Carlo (MC),
...
change during the calculation. FLUKE also leaves some restart files in the
...
(PIMC or reaction path) simulation is started FLUKE checks the working
...
The SD algorithm in FLUKE does not use a line search to optimize $\alpha$ at
...
The DFP algorithm can update the inverse Hessian using the forces ($F$), the
current Hessian, and the optimization vector ($\vec p$). The optimization
algorithm is given by

\begin{equation}
 \vec p_n = \alpha_n H_n^{-1} F_n \; ,
\end{equation}
\begin{equation}
 \delta_n = F_{n-1}-F_n \; ,
\end{equation}
...
\begin{equation}
 \label{eq:DFP}
 H_{n+1}^{-1} = H_n^{-1} + \frac{\vec p_n \vec p_n^T}{\vec p_n^T \delta_n} -
...
To keep the FLUKE DFP optimizer simple, the guess inverse Hessian is chosen
